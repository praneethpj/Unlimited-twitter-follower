...
function checkiffollowing(ele)
{
	var followstat=	ele.innerText
 
	if (followstat=='Follow back') 
		return true
	else
		return false
	return false
...
function advanced_tests(ele){
 
	if(follow_non_followers=="Yes")
		return (!checkiffollowing(ele))
	return true
}
...
chrome.storage.local.get({sd_options_storage:'{}'}, function(result) {
          var obj=JSON.parse(result.sd_options_storage)
			...
			follow_non_followers=obj['follow_non_followers']
